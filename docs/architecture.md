...
- **Справка / первый запуск:** [`EditorGuideModal.vue`](../packages/helios-editor/src/ui/EditorGuideModal.vue) при первом визите (если не отмечено «Не показывать» в `localStorage`); кнопка **?** справа на верхней полосе центра. Настройка: **`createEditor({ welcomeGuide: { extraSections?, sections?, storageKey? } })`** — в Astris доп. секции GoL в [`examples/astris/src/editorWelcomeGuide.ts`](../examples/astris/src/editorWelcomeGuide.ts).
...
- **Корабль (Play):** на маркере `ModelInstance` можно повесить `ShipOrbit`, `ShipBob`, `ShipSway` — при expand переносятся на wrapper; системы орбиты/покачивания/крена (`runsInEditor = false`).
