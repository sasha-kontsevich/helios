...
- **Game UI overlay:** `createEditor({ gameUiPlugins })` → `GameUiHost` + `.shell__gameUiRoot` над `#helios-game-view`; Astris — full cockpit (`astrisGameUi.css`, Play/Pause, Paint/Erase, Clear, presets, LMB drag).
