...
/** Minimum segment counts for stable geometry (Phase1 validation). */
...
function clampPlaneSeg(n: unknown, fallback: number): number {
    if (typeof n !== 'number' || !Number.isFinite(n)) return Math.max(MIN_PLANE_SEGMENTS, Math.floor(fallback));
